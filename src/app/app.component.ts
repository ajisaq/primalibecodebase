...
export class AppComponent {
  ...
  ngOnInit() {
    this.airportsDropdownService
      .getAirportsDropdown()
      .then((airportsDropdown: any[]) => {
    ...
    this.cabinClassesDropdownService
      .getCabinClassesDropdown()
      .then((cabinClassesDropdown: any[]) => {
        this.cabinClassesDropdown = cabinClassesDropdown;

        this.cabinClassesDropdown.forEach((cabinClassDropdown: any) => {
          this.indexedCabinClasses[cabinClassDropdown.extra.code] =
            cabinClassDropdown.extra;
        });
        // console.log('indexed cabin classes', this.indexedCabinClasses);
        this.batService.onIndexedCabinClassesChanged.next(
          this.indexedCabinClasses
        );
      });
    this.bookingClustersService
      .getBookingClusters()
      .then((bookingClusters: any[]) => {
        ...
        this.batService.onIndexedBookingClustersChanged.next(
          indexedBookingClusters
        );
      });
    this.passengerTypesDropdownService
      .getPassengerTypesDropdown()
      .then((passengerTypesDropdown: any[]) => {
        ...
        this.passengerTypesDropdown.forEach((passengerTypeDropdown: any) => {
          indexedPassengerTypesDropdown[passengerTypeDropdown.value] =
            passengerTypeDropdown.extra;
        ...
        this.batService.onIndexedPassengerTypesChanged.next(
    ...
    this.batService.onBookingFormChanged
      .pipe()
      .subscribe((bookingForm: any) => (this.bookingForm = bookingForm));
    this.batService.onCreatePNRButtonClicked
      .pipe()
      .subscribe((clicked: boolean) => this.onSavePNR());
  }
  ...
  createBookingForm(booking?: Booking) {
    ...
    return this.fb.group({
      id: [booking.id],
      date: [
        booking.date ? moment(booking.date, 'DD/MM/YYYY HH:mm') : moment(),
      ],
      effectiveDate: [
        booking.effectiveDate
          ? moment(booking.effectiveDate, 'DD/MM/YYYY HH:mm')
          : moment(),
      ],
  ...
  getBookingLegsFormArray(bookingLegs: any) {
    ...
    bookingLegs.forEach((bookingLeg: BookingLeg) => {
      let el = this.fb.group<any>({
        ...
        //TODO
        bookingLegStops: [
          bookingLeg.bookingLegStops.map((bStop) => new ScheduleLeg(bStop).iri),
        ],
    ...
    coupons.forEach((coupon: Coupon | any) => {
      resultArray.push(
        this.fb.group({
          ...
          bookedProducts: this.getBookedProductsFormArray(
            coupon.bookedProducts
          ),
  ...
  getBookingLegStopFaresFormArray(
    bookingLegStopFares: BookingLegStopFare[] | any[]
  ) {
    // let bookingLegs = booking_legs; //booking_legs.map(booking_leg=>new BookingLeg(booking_leg))
    let resultArray = this.fb.array([]) as FormArray;
    bookingLegStopFares.forEach(
      (bookingLegStopFare: BookingLegStopFare | any) => {
        resultArray.push(
          this.fb.group({
  ...
  onSavePNR() {
    ...
    bookingLegs.controls.forEach((bookingLeg: any) => {
      ...
      let cabinClassIri =
        this.indexedCabinClasses[bookingLeg.controls['cabinClass'].value]['@id'];
    ...
    if (!id) {
      ...
      //   debugger
      this.batService
        .updateBooking(updatedBookingForm.getRawValue())
        .then((booking) => {
          //open Saved booking
