...
export class ManageBookingComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.paramMap.subscribe((params: any) => {
      ...
      if (this.searchedPNR){ 
        ...
        this.batService.getPNRFromSearch2(this.searchedPNR).then((booking: Booking)=> {
  ...
  selectSeat(){
  this.router.navigate(['/manage-booking/select-seat/', this.flightId, this.passengerInfo.pnr])
}
  get bookingLegs() {
  ...
  sendTicketToEmail(){
    this.isLoading = true;
      this.batService.sendEmailTicket(this.passengerInfo.pnr, 'talk2ahmedpeter@gmail.com')
      .then((response: any)=>{
  ...
  getFlightSummary(booking: any){
    this.flightSummaryDetails = [];
          ...
          booking.bookingLegs.map((bookingLeg: any)=>{
              ...
              let duration = (moment(bookingLeg.flight?.arrive_date, 'YYYY-MM-DD\THH:mm:ss').diff(moment(bookingLeg.flight?.depart_date, 'YYYY-MM-DD\THH:mm:ss')));
  ...
  changeFlight(data: any){
    this.router.navigate([`manage-booking/${this.passengerInfo.pnr}/change-flight`], {queryParams:data, queryParamsHandling: 'merge'})
  }
